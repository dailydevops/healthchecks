...
- `tags`: The tags for the health check. The tags `postgresql`, `database` and `devart` are always used as default and combined with the user input. You can provide additional tags to group or filter the health checks.
...
The second approach is to use the builder based approach. This approach is recommended if you have only one PostgreSQL instance to check or dynamic programmatic values.
```csharp
var builder = services.AddHealthChecks();

builder.AddNpgsqlDevart("<name>", options =>
{
...
builder.AddNpgsqlDevart("<name>", options => ..., "postgresql", "database");
```
