...
internal sealed class CosmosContainerAvailableConfigure
    : IConfigureNamedOptions<CosmosContainerAvailableOptions>,
        IValidateOptions<CosmosContainerAvailableOptions>
{
    private readonly IConfiguration _configuration;
    private readonly IServiceProvider _serviceProvider;

    public CosmosContainerAvailableConfigure(IConfiguration configuration, IServiceProvider serviceProvider)
    {
        _configuration = configuration;
        _serviceProvider = serviceProvider;
    }

    public void Configure(string? name, CosmosContainerAvailableOptions options)
    {
        Argument.ThrowIfNullOrWhiteSpace(name);
        _configuration.Bind($"HealthChecks:AzureCosmosContainer:{name}", options);
    }

    public void Configure(CosmosContainerAvailableOptions options) => Configure(Options.DefaultName, options);

    public ValidateOptionsResult Validate(string? name, CosmosContainerAvailableOptions options)
    {
        ...
        {
            return Fail("The container ID cannot be null or whitespace.");
        }
    ...
    private static ValidateOptionsResult ValidateModeDefaultAzureCredentials(CosmosContainerAvailableOptions options)
    {
    ...
    private static ValidateOptionsResult ValidateModeConnectionString(CosmosContainerAvailableOptions options)
    {
