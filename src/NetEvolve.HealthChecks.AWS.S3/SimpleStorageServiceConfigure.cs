...
internal sealed class SimpleStorageServiceConfigure
    : IConfigureNamedOptions<SimpleStorageServiceOptions>,
        IValidateOptions<SimpleStorageServiceOptions>
{
    private readonly IConfiguration _configuration;

    public SimpleStorageServiceConfigure(IConfiguration configuration) => _configuration = configuration;

    public void Configure(string? name, SimpleStorageServiceOptions options)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(name);
        _configuration.Bind($"HealthChecks:AWSS3:{name}", options);
    }

    public void Configure(SimpleStorageServiceOptions options) => Configure(Options.DefaultName, options);

    public ValidateOptionsResult Validate(string? name, SimpleStorageServiceOptions options)
    {
