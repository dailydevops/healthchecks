...
This package provides a health check for Minio, based on the [Minio](https://www.nuget.org/packages/Minio/) package.
...
The health check is a liveness check. It checks if the Minio Server is available and if the bucket exists.
...
The first one is to use the configuration-based approach. This approach is recommended if you have multiple Minio instances to check.
```csharp
