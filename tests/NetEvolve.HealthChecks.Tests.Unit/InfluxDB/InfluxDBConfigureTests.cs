...
public sealed class InfluxDBConfigureTests
{
    [Test]
    public async Task Configure_WithNameAndOptions_BindsConfigurationCorrectly()
    {
        ...
        {
            ["HealthChecks:InfluxDB:TestName:KeyedService"] = "test-key",
            ["HealthChecks:InfluxDB:TestName:Timeout"] = "200",
        };

        var configuration = new ConfigurationBuilder().AddInMemoryCollection(configValues).Build();

        var options = new InfluxDBOptions();
        var configure = new InfluxDBConfigure(configuration);
    ...
    {
        // Arrange
        var configure = new InfluxDBConfigure(new ConfigurationBuilder().Build());
    ...
    public async Task Validate_WhenArgumentOptionsNull_ThrowArgumentNullException()
    {
        ...
        var options = default(InfluxDBOptions);
    ...
    public async Task Validate_WhenArgumentTimeoutLessThanInfinite_ThrowArgumentException()
    {
        ...
        var configure = new InfluxDBConfigure(new ConfigurationBuilder().Build());
        const string? name = "Test";
        var options = new InfluxDBOptions { Timeout = -2 };
    ...
    [Test]
    public async Task Validate_WhenArgumentPingAsyncNull_SetDefaultPingAsync()
    {
        // Arrange
        var configure = new InfluxDBConfigure(new ConfigurationBuilder().Build());
        const string? name = "Test";
        ...
        using (Assert.Multiple())
        {
            ...
            _ = await Assert.That(options.PingAsync).IsEqualTo(InfluxDBHealthCheck.DefaultPingAsync);
        }
    }
}
