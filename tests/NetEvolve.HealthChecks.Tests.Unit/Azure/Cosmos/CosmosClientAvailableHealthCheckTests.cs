...
public sealed class CosmosClientAvailableHealthCheckTests
{
    [Fact]
    public async Task CheckHealthAsync_WhenContextIsNull_ThrowsArgumentNullException()
    {
        ...
        var optionsMonitor = serviceProvider.GetRequiredService<IOptionsMonitor<CosmosClientAvailableOptions>>();
        var sut = new CosmosClientAvailableHealthCheck(serviceProvider, optionsMonitor);
    ...
    public async Task CheckHealthAsync_WhenOptionsProvided_ExecutesHealthCheck()
    {
        ...
        var services = new ServiceCollection();
        _ = services.AddOptions();
        _ = services.Configure<CosmosClientAvailableOptions>(
            "test",
            options =>
            {
