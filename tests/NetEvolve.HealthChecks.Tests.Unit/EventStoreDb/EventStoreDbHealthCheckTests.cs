...
public sealed class EventStoreDbHealthCheckTests
{
    ...
    public async Task CheckHealthAsync_WithKeyedService_ShouldUseKeyedService()
    {
        ...
        var client = Substitute.For<EventStoreClient>();

        var serviceCollection = new ServiceCollection();
        _ = serviceCollection.AddKeyedSingleton("test-key", client);
        var serviceProvider = serviceCollection.BuildServiceProvider();

        var healthCheck = new EventStoreDbHealthCheck(serviceProvider, optionsMonitor);
    ...
    public async Task CheckHealthAsync_WhenConnectionFails_ShouldReturnUnhealthy()
    {
        ...
        // Setup client mock that throws an exception
        var client = Substitute.For<EventStoreClient>();
