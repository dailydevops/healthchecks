...
public sealed class BigQueryConfigureTests
{
    [Test]
    public void Configure_WhenArgumentNameNull_ThrowArgumentNullException()
    {
        // Arrange
        var configure = new BigQueryOptionsConfigure(new ConfigurationBuilder().Build());
    ...
    public async Task Configure_WhenConfigurationValid_OptionsConfigured()
    {
        ...
        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(
                ...
                {
                    { "HealthChecks:GCP:BigQuery:Test:Timeout", "5000" },
                    { "HealthChecks:GCP:BigQuery:Test:KeyedService", "my-bigquery" },
                }
            )
            .Build();

        var configure = new BigQueryOptionsConfigure(config);
        var options = new BigQueryOptions();
        ...
        using (Assert.Multiple())
        {
            ...
            _ = await Assert.That(options.KeyedService).IsEqualTo("my-bigquery");
        }
    }
    ...
    public async Task Configure_WhenTimeoutNegative_ValueSet()
    {
        ...
        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(
                new Dictionary<string, string?> { { "HealthChecks:GCP:BigQuery:Test:Timeout", "-1" } }
            )
            .Build();
    ...
    public async Task Validate_WhenTimeoutLessThanMinusOne_ReturnsFailure()
    {
        ...
        var config = new ConfigurationBuilder().Build();
        var configure = new BigQueryOptionsConfigure(config);
        var options = new BigQueryOptions { Timeout = -2 };
    ...
    public async Task Validate_WhenTimeoutValid_ReturnsSuccess()
    {
        ...
        var config = new ConfigurationBuilder().Build();
        var configure = new BigQueryOptionsConfigure(config);
        var options = new BigQueryOptions { Timeout = 1000 };
